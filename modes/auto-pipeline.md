...
If the input is a **URL** (not pasted JD text), use the following strategy to extract the content:

**Priority order:**

1. **Playwright (preferred):** Most job portals (Lever, Ashby, Greenhouse, Workday) are SPAs. Use `browser_navigate` + `browser_snapshot` to render and read the JD.
2. **WebFetch (fallback):** For static pages (ZipRecruiter, WeLoveProduct, company career pages).
3. **WebSearch (last resort):** Search for the role title + company on secondary portals that index JDs as static HTML.

**If none of the above works:** Ask the candidate to paste the JD manually or share a screenshot.

**If the input is already JD text** (not a URL): use it directly, no fetch needed.
...
1. **Extract form questions**: Use Playwright to navigate to the form and take a snapshot. If unavailable, use the generic questions below.
2. **Generate answers** following the tone guidelines below.
3. **Append to the report** as section `## G) Draft Application Answers`.
...
- **Lead with proof, not assertion**: Instead of "I'm great at X", say "I built X that does Y."
